...
.. module:: engine
   :platform: Unix
   :synopsis: Definition of object tracker
...
import logging
logging.basicConfig()
logger = logging.getLogger('Object-Tracker')
logger.setLevel(logging.INFO)

from scipy.spatial.distance import euclidean

class OpenCV_Tracker(base_object_estimator.BaseObjectEstimator):
# class OpenCV_Tracker():
    ...
    def __init__(self, **kw):
        ...
        self.objects = list()
        self.object_life_cycle = kw.get('object_life_cycle', 5)
        self.detector = kw.get('detector', None)
        self.detection_rate = kw.get('detection_rate', 10)
        self.tracker_name = kw.get('tracker_name', 'csrt')
        self.frame_idx = 0
        ...
        assert issubclass(type(
            self.detector), base_object_estimator.BaseObjectEstimator), "Object detector must inherit BaseObjectEstimator"
    ...
    def process(self, frame):
        ...
        if (self.frame_idx % self.detection_rate) == 0:  # Detect objects
            ...
            dead_objects = self._update_from_bbox(
                # frame, [obj['detection_bbox_original'] for obj in detected_objects])
                frame, [obj.bbox for obj in detected_objects])

        else:
            logger.info("Tracking {} objects".format(
                len(self.objects)))
            self.frame_idx += 1
    ...
    def _update_from_bbox(self, frame, bboxes):
        ...
        for bbox in bboxes:
            ...
            if min_dist > max_distance:
                ...
                new_objects.append(base_object_estimator.TrackingObject(
                    bbox[0], bbox[1], bbox[2], bbox[3],
                    life=self.object_life_cycle,
                    start_time=start_time,
                    tracker=self.tracker_list[self.tracker_name]()))

                new_objects[-1].tracker.init(frame,
                                             (
                                                 new_objects[-1].bbox_xmin,
                                                 new_objects[-1].bbox_ymin,
                                                 new_objects[-1].bbox_xmax -
                                                 new_objects[-1].bbox_xmin,
                                                 new_objects[-1].bbox_ymax -
                                                 new_objects[-1].bbox_ymin
                                             ))
                ...
                self.objects[nearest_tracking_obj].update_bbox(
                    bbox[0], bbox[1], bbox[2], bbox[3])
    ...
    def _update_from_tracker(self, frame):
        ...
        for obj in self.objects:
            ...
            if success:
                obj.update_bbox(int(pos[0]), int(pos[1]),
                                int(pos[0] + pos[2]), int(pos[1] + pos[3]))

    def _remove_dead_objects(self):
        """ Deallocate dead objects to stop tracking them
        """
        dead_objects_idx = np.where(
            np.array([tracker.life for tracker in self.objects]) == 0)[0]

        dead_objects = []
        if len(dead_objects_idx) > 0:
            logger.info('Deleting {} dead objects'.format(
                len(dead_objects_idx)))
            
            end_time = datetime.now()
            for i in sorted(dead_objects_idx, reverse=True):
                offset = (
                    end_time - self.objects[i].start_time).total_seconds()
                dead_objects.append({'timestamp_start': self.objects[i].start_time,
                                     'offset': offset})
